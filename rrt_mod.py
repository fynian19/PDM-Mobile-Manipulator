...
from IK_def import IK_endpoint2
...
from pyroboplan.visualization.meshcat_utils import visualize_frames, visualize_paths
...
class RRTPlanner2:
    ...
    def __init__(self, model, collision_model, options=RRTPlannerOptions2()):
        """
    ...
    def plan(self, q_start, q_goal, l1, l2, l3, l4, exclusion_radius, end_point):
        """
        ...
        if check_collisions_at_state(
            ...
            return None
        while check_collisions_at_state(
            self.model,
            self.collision_model,
            q_goal,
            self.data,
            ...
            print("Goal configuration in collision. Calculating new goal configuration.")
            #return None
            q_goal = IK_endpoint(l1, l2, l3, l4, exclusion_radius, end_point, upper=True)
            print(f"New END: {q_goal}")
        ...
        goal_candidates = []  # for storing valid end nodes in case fast return is false, set up for unidirectional RRT/RRT*
        
        while True:
            n = len(self.start_tree.nodes)  # amount of current nodes
            self.max_rewire_dist = min(self.options.max_connection_dist, self.options.max_connection_dist * (math.log(n) / n)**(1/5))  # 1/d = 1/5, 5 dimensions
            print(f"Number of current nodes: {n}")
            print(f"New rewiring distance: {self.max_rewire_dist}")
            ...
            if goal_found and self.options.fast_return:
                ...
                print(f"Time spent planning: {time.time() - t_start}s")
                break
            ...
            if np.random.random() < self.options.goal_biasing_probability:
                ...
                base_x, base_y, base_q = q_sample[0], q_sample[1], q_sample[2]  # for getting new q_goal
                base_coord = [base_x, base_y, l1]
                for i in range(1000):
                    #print(i)
                    q_goal_prel = IK_endpoint2(l1, l2, l3, l4, base_q, exclusion_radius, base_coord, end_point, upper=True)
                    #print(q_goal_prel)
                    if q_goal_prel is not None:
                        if not check_collisions_at_state(self.model,self.collision_model,
                        q_goal_prel,self.data,self.collision_data,
                        distance_padding=self.options.collision_distance_padding,):
                            q_goal = q_goal_prel
                            latest_goal_tree_node = q_goal_prel
                            self.goal_tree.add_node(Node(q_goal_prel, parent=None, cost=0.0))
                            print(f'New goal configuration: {q_goal}')
                            break
                ...
                if distance_to_other_tree <= self.options.max_connection_dist:
                    ...
                    if not check_collisions_along_path(
                        ...
                        if distance_to_other_tree > 0:
                            ...
                            print(f"Connected to: {nearest_node_in_other_tree.q}")
                            print(f"Created node: {new_node}")
                            #print(f"Actual goal:  {q_goal}")
                        if start_tree_phase:
                            latest_start_tree_node = new_node
                            latest_goal_tree_node = nearest_node_in_other_tree
                            print('ADDING ADDING ADDING')
                            goal_candidates.append([new_node, new_node.cost])  # keeps track of valid end nodes, because q_goal changes otherwise
                        else:
        ...
        best_node = min(goal_candidates, key=lambda pair: pair[1])[0]
        latest_start_tree_node = best_node
        latest_goal_tree_node = None  # we only care about one directional RRT here for our use case
